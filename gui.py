...
class BareboneBuilder:
    ...
    def build_kernel(self):
        ...
        fff=f'pcc -c "$1" -o /tmp/kernel.o -nostdlib'.replace("$1",filename)
        ...
        self.execute_command("dd if=/tmp/hellos.com of=/tmp/boot.bin seek=1 conv=notrunc",True)
        self.execute_command("cp -f /tmp/boot.bin ./",True)
    def run_kernel(self):
        self.text_area.delete(1.0, tk.END)
        self.execute_command("/usr/bin/qemu-system-x86_64 -boot c -hda boot.bin",True)
